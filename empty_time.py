...
import faiss
from database_tables import Hobby, Checklist, Schedule, session_scope
class CalendarRecommendation:
    ...
    def generate_recommendations_data(self, date):
        ...
        for start, end, activity in recommendations:
            ...
            if activity_type == '할 일':
                recommendation = {
                    "user_id": self.user_id,  # user_id를 클래스 속성으로 가정
                    "date": date,
                    "subtitle": activity_name,
                    "content": activity_name,
                    "start_time": datetime.datetime.combine(date, start),
                    "end_time": datetime.datetime.combine(date, end),
                    "is_loop": False,
                    "location": "위치 없음",
                    "is_alarm": False,
                    "alarm_time": None
                }
            elif activity_type == '취미':
                recommendation = {
                    "user_id": self.user_id,
                    "name": activity_name,
                    "preferred_time_start": start,
                    "preferred_time_end": end,
                    "count": 1,
                    "play_time": (datetime.datetime.combine(date, end) - datetime.datetime.combine(date, start)).total_seconds() / 3600,
                    "last_done_date": date
                }
            
            recommendation_data.append((activity_type, recommendation))
        
        return recommendation_data

    def save_recommendations_to_db(self, date):
        recommendations = self.generate_recommendations_data(date)

        with session_scope() as session:
            
            for activity_type, rec in recommendations:
                if activity_type == '할 일':
    ...
    def print_recommendations(self, date):
        ...
        if not recommendations:
            ...
            for activity_type, rec in recommendations:
                if activity_type == '할 일':
                    print(f"  {rec['start_time'].strftime('%H:%M')} - {rec['end_time'].strftime('%H:%M')} "
                        f"({(rec['end_time'] - rec['start_time']).total_seconds() / 3600:.1f}시간): "
                        f"할 일: {rec['subtitle']}")
                elif activity_type == '취미':
                    print(f"  {rec['preferred_time_start'].strftime('%H:%M')} - {rec['preferred_time_end'].strftime('%H:%M')} "
                        f"({rec['play_time']:.1f}시간): 취미: {rec['name']}")
